...
//        mCyclePaint.setStyle(Paint.Style.FILL_AND_STROKE);
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        Log.e(TAG,"-mWaveLength + mOffset    "+(-mWaveLength + mOffset));
        Log.e(TAG,"mCenterY    "+mCenterY);
        Log.e(TAG,"mWaveCount    "+mWaveCount);
        for (int i = 0; i < mWaveCount; i++)
        {
            //正弦曲线
            mPath.quadTo((-mWaveLength * 3 / 4) + (i * mWaveLength) + mOffset, mCenterY + 60, (-mWaveLength / 2) + (i * mWaveLength) + mOffset, mCenterY);
            Log.e(TAG,"(-mWaveLength * 3 / 4) + (i * mWaveLength) + mOffset    "+((-mWaveLength * 3 / 4) + (i * mWaveLength) + mOffset));
            Log.e(TAG,"mCenterY + 60    "+(mCenterY + 60));
            Log.e(TAG,"(-mWaveLength / 2) + (i * mWaveLength) + mOffset    "+((-mWaveLength / 2) + (i * mWaveLength) + mOffset));
            Log.e(TAG,"mCenterY    "+(mCenterY ));
            mPath.quadTo((-mWaveLength / 4) + (i * mWaveLength) + mOffset, mCenterY - 60, i * mWaveLength + mOffset, mCenterY);
            Log.e(TAG,"(-mWaveLength / 4) + (i * mWaveLength) + mOffset    "+((-mWaveLength / 4) + (i * mWaveLength) + mOffset ));
            Log.e(TAG,"mCenterY - 60    "+(mCenterY - 60));
            Log.e(TAG,"i * mWaveLength + mOffset    "+(i * mWaveLength + mOffset));
            Log.e(TAG,"mCenterY    "+(mCenterY));
